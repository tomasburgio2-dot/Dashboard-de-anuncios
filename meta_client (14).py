...
def _pick_result(actions: list[dict] | None, sql_action_type: str | None) -> tuple[str, float]:
    ...
    A propósito NO se usa "offsite_conversion.fb_pixel_custom" como
    respaldo: la documentación de Meta confirma que ese es un balde
    genérico donde se agrupan TODOS los eventos personalizados del
    píxel que no estén formalizados como "conversión personalizada" en
    Events Manager, sin forma de diferenciarlos vía API — probado con
    datos reales de esta cuenta (34 vs. 18 real). Mostrar 0 cuando no se
    encuentra la conversión formal es más fiel que mostrar un número
    inflado con eventos que no son SQL.

    Para que esto muestre el número correcto, hace falta crear una
    "conversión personalizada" formal en Events Manager para el evento
    SQL (ver README) — una vez que exista, se detecta sola por nombre."""
