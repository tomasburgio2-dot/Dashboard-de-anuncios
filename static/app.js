...
function money(value) {
  return new Intl.NumberFormat(LOCALE[state.market], {
    style: "currency",
...
async function fetchData() {
  ...
  try {
    const res = await fetch(
      `/api/creatives?market=${state.market}&range=${state.range}`,
    ...
    state.currency = json.currency || "USD";
    ...
    const message =
      err.name === "AbortError"
        ? "Meta tardó demasiado en responder. Probá de nuevo en un momento."
...
function renderSummary(rows) {
  // El total de arriba viene directo del backend a nivel campaña (mismo
  // número que la pestaña "Campañas" de Meta) — a propósito NO se suman
  // las cards de abajo, porque Meta documenta que esa suma puede no
  // coincidir con el total real cuando una misma conversión se atribuye
  // a más de un anuncio dentro de la campaña.
  const s = state.summary;
  const totalSpend = s ? s.spend : rows.reduce((sum, r) => sum + r.spend, 0);
  const totalResults = s ? s.sql : rows.reduce((sum, r) => sum + (r.result_count || 0), 0);
  const avgCpr = s ? s.cost_per_result : (totalResults ? totalSpend / totalResults : null);

  summaryEl.innerHTML = `
    <div class="item"><span class="label">Creativos</span><span class="value">${rows.length}</span></div>
    <div class="item"><span class="label">Gastado</span><span class="value">${money(totalSpend)}</span></div>
    <div class="item"><span class="label">SQL</span><span class="value">${number(totalResults)}</span></div>
    <div class="item"><span class="label">Costo por resultado</span><span class="value">${avgCpr ? money(avgCpr) : "—"}</span></div>
  `;
}
  ...
  return `
    <article class="card">
      <div class="card-thumb">
        ...
        <div class="cost-tag">
          <span class="n">${row.cost_per_result ? money(row.cost_per_result) : "—"}</span>
      ...
      <div class="card-body">
        ...
        <div class="metrics">
          <div class="metric"><span class="label">Gastado</span><span class="value">${money(row.spend)}</span></div>
          <div class="metric"><span class="label">${resultLabel}</span><span class="value">${number(row.result_count)}</span></div>
          <div class="metric"><span class="label">Clics</span><span class="value">${number(row.clicks)}</span></div>
          <div class="metric"><span class="label">CTR</span><span class="value">${number(row.ctr, 2)}%</span></div>
          <div class="metric"><span class="label">CPC</span><span class="value">${money(row.cpc)}</span></div>
        </div>
      </div>
    </article>
  `;
