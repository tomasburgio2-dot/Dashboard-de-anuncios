...
def _pick_result(actions: list[dict] | None, sql_action_type: str | None) -> tuple[str, float]:
    ...
    1) la conversión personalizada formal "SQL", cuando Meta la indexó
       para ese anuncio/día — el número exacto, sin tocar.
    2) "offsite_conversion.fb_pixel_custom" dividido por 2. Ese balde
       junta el evento SQL con otras etapas del embudo de HubSpot
       (opportunity, customer, etc.) y, según lo comparado contra Ads
       Manager, viene aproximadamente al doble del SQL real — es una
       corrección a ojo basada en un solo punto de comparación, no una
       fórmula garantizada; puede desviarse si la mezcla de etapas
       cambia día a día.
    Si ninguna de las dos aparece, es 0 real."""
