...
def _pick_result(actions: list[dict] | None, sql_action_type: str | None) -> tuple[str, float]:
    ...
    Lead (SQL). Prioridad:
    1) la conversión personalizada formal "SQL", cuando Meta la usó para
       ese anuncio puntual (a veces sí, a veces no — confirmado con
       datos reales de esta cuenta);
    2) "offsite_conversion.fb_pixel_custom" — el balde genérico donde
       Meta agrupa el evento cuando NO usó la conversión formal. Se
       comprobó con datos reales que, en esta cuenta, es prácticamente
       siempre el mismo evento SQL el que cae ahí — así que es una
       aproximación razonable, mejor que mostrar 0 sabiendo que hubo
       SQL real.
    Si ninguna de las dos aparece, es 0 real (el anuncio no generó SQL
    en el período)."""
