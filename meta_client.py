...
def _get_sql_action_type() -> str | None:
    """Busca en la cuenta una conversión personalizada cuyo nombre
    contenga 'sql' (Sales Qualified Lead) y devuelve el action_type que
...
def _pick_result(actions: list[dict] | None, sql_action_type: str | None) -> tuple[str | None, float]:
    """Devuelve (tipo_de_resultado, cantidad) para un anuncio. Si la cuenta
    tiene una conversión personalizada de SQL, esa es siempre la que manda;
    si no, cae a RESULT_ACTION_PRIORITY como plan B."""
    if not actions:
        return None, 0.0
...
def _fetch_creatives(account_id: str, ad_ids: list[str]) -> dict:
    ...
    for i in range(0, len(ad_ids), chunk_size):
        ...
        data = _get(
            f"act_{account_id}/ads",
            {
                "fields": (
                    ...
                    # El modificador .thumbnail_width().thumbnail_height()
                    # tiene que ir pegado al campo "creative" (no como
                    # parámetro suelto) para que realmente afecte el
                    # thumbnail_url anidado — si no, Meta ignora el tamaño
                    # pedido y devuelve el de 64x64 por default, que se ve
                    # borroso al estirarlo en una card grande.
                    "creative.thumbnail_width(720).thumbnail_height(720)"
                    "{thumbnail_url,image_url,object_type,"
                    "object_story_spec{video_data{image_url}},asset_feed_spec}"
                ),
                "filtering": str(
        ...
        for row in data.get("data", []):
            ...
            # Prioridad: imagen completa (fotos) > portada del video
            # (videos) > miniatura chica como último recurso.
            thumb = creative.get("image_url") or video_cover or creative.get("thumbnail_url")
            out[row["id"]] = {
                "thumbnail_url": thumb,
