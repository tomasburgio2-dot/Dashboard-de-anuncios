...
def _pick_result(
    ...
    conversions: list[dict] | None,
    sql_action_type: str | None,
) -> tuple[str, float]:
    """Devuelve (tipo_de_resultado, cantidad) para un anuncio. Prioridad:
    1) el campo `conversions` de la API, que — a diferencia de `actions` —
       desglosa los eventos personalizados POR NOMBRE (ej.
       "offsite_conversion.fb_pixel_custom.salesqualifiedlead"). Es con
       toda probabilidad el mismo dato que muestra la columna Resultados
       de Ads Manager: exacto, sin mezclar etapas del embudo.
    2) la conversión personalizada formal "SQL" por ID, si Meta la
       indexó en `actions` para ese anuncio/día.
    3) el balde genérico "offsite_conversion.fb_pixel_custom" dividido
       por 2 — aproximación de último recurso (el balde junta SQL con
       otras etapas del embudo de HubSpot y venía ~al doble).
    Si nada de eso aparece, es 0 real."""
    for c in conversions or []:
        normalized = c.get("action_type", "").lower().replace("_", "")
        if "salesqualifiedlead" in normalized or (
            sql_action_type and c.get("action_type") == sql_action_type
        ):
            return "sql", float(c.get("value", 0))
    ...
    if "offsite_conversion.fb_pixel_custom" in by_type:
        return "sql", round(by_type["offsite_conversion.fb_pixel_custom"] / 2)
...
def _fetch_insights(account_id: str, date_preset: str) -> dict:
    ...
    fields = (
        ...
        "clicks,ctr,cpc,cpm,frequency,actions,conversions"
    )
    data = _get(
    ...
    for row in data.get("data", []):
        result_type, result_count = _pick_result(
            row.get("actions"), row.get("conversions"), sql_action_type
        )
...
def get_debug_sql_resolution(market: str, range_key: str) -> dict:
    """SOLO DIAGNÓSTICO: expone el razonamiento interno completo — qué
    sql_action_type resuelve _get_sql_action_type() en este momento (con
    su caché tal cual está, no una llamada limpia), y para cada anuncio
    con gasto real, si esa clave aparece en sus acciones o no. Esto
    debería dejar a la vista exactamente dónde se corta la cadena."""
    ...
    data = _get(
        f"act_{AD_ACCOUNT_ID}/insights",
        {
            ...
            "fields": "ad_id,ad_name,campaign_name,spend,actions,conversions",
            "limit": 500,
        },
    )
    rows = [
        ...
        and float(r.get("spend", 0) or 0) > 0
    ]
    rows.sort(key=lambda r: float(r.get("spend", 0) or 0), reverse=True)
    rows = rows[:6]

    out = []
    for r in rows:
        by_type = {a["action_type"]: a["value"] for a in r.get("actions", [])}
        out.append(
            {
                "ad_name": r.get("ad_name"),
                "spend": r.get("spend"),
                "esta_presente": bool(sql_action_type) and sql_action_type in by_type,
                "valor_si_esta": by_type.get(sql_action_type) if sql_action_type else None,
                "todas_las_action_types": list(by_type.keys()),
            }
        )
    return {"sql_action_type_detectado": sql_action_type, "anuncios": out}


def get_debug_campaign_raw(range_key: str) -> dict:
    """SOLO DIAGNÓSTICO: la respuesta CRUDA de Meta a nivel campaña, sin
    ningún procesamiento — para ver si hay filas duplicadas o algo raro
    que explique un resumen doblado."""
    if not AD_ACCOUNT_ID:
        raise MetaAPIError("Falta META_AD_ACCOUNT_ID.")
    if range_key not in DATE_PRESETS:
        raise MetaAPIError(f"Rango de fecha desconocido: {range_key}")
    return _get(
        f"act_{AD_ACCOUNT_ID}/insights",
        {
            "level": "campaign",
            "date_preset": DATE_PRESETS[range_key],
            "fields": "campaign_id,campaign_name,spend,actions,conversions",
            "limit": 500,
        },
    )


def get_debug_custom_conversions() -> dict:
    """SOLO DIAGNÓSTICO: llama directo a customconversions y devuelve el
    resultado CRUDO —incluyendo el error tal cual si falla—, porque
    _get_sql_action_type() traga cualquier error silenciosamente y eso
    puede estar escondiendo un problema de permisos."""
    if not AD_ACCOUNT_ID:
        raise MetaAPIError("Falta META_AD_ACCOUNT_ID.")
    data = _get(f"act_{AD_ACCOUNT_ID}/customconversions", {"fields": "id,name"})
    return data
...
def get_debug_promoted_objects(market: str) -> list[dict]:
    """SOLO DIAGNÓSTICO: trae, para cada conjunto de anuncios del
    mercado, el promoted_object — ahí Meta guarda EXACTAMENTE a qué
    evento o conversión optimiza (vive en el ad set, no en la campaña),
    en vez de que el código lo adivine a partir de las acciones."""
    if market not in MARKET_PREFIXES:
        raise MetaAPIError(f"Mercado desconocido: {market}")
    if not AD_ACCOUNT_ID:
        raise MetaAPIError("Falta META_AD_ACCOUNT_ID.")
    ...
    cdata = _get(
        ...
        {"fields": "id,name,objective", "limit": 200},
    )
    campaigns = [
        c for c in cdata.get("data", [])
        if c.get("name", "").upper().startswith(prefix)
    ]
    campaign_name_by_id = {c["id"]: c["name"] for c in campaigns}
    campaign_ids = list(campaign_name_by_id.keys())
    if not campaign_ids:
        return []
    ...
    adata = _get(
        ...
        {
            "fields": "name,campaign_id,optimization_goal,promoted_object",
            "filtering": str(
    ...
    return [
        {
            "campaign_name": campaign_name_by_id.get(a.get("campaign_id"), "?"),
            "adset_name": a.get("name"),
            "optimization_goal": a.get("optimization_goal"),
            "promoted_object": a.get("promoted_object"),
        }
        for a in adata.get("data", [])
    ]


def get_debug_sql_by_ad(market: str, range_key: str) -> list[dict]:
    """SOLO DIAGNÓSTICO (temporal): SQL calculado por el dashboard para
    CADA anuncio del mercado/rango pedido, para comparar uno por uno
    contra la pestaña "Anuncios" de Ads Manager y encontrar exactamente
    dónde está la diferencia, en vez de seguir adivinando la causa."""
    rows = get_creative_performance(market, range_key)
    return sorted(
        [
            {
                "ad_name": r["ad_name"],
        ...
        key=lambda r: r["ad_name"],
    )


def _fetch_campaign_level(range_key: str) -> list[dict]:
    """Totales A NIVEL CAMPAÑA (gasto + SQL), igual que la pestaña
    "Campañas" de Ads Manager. Se usa para el resumen de arriba en vez de
    sumar las filas a nivel anuncio: Meta documenta que la suma a nivel
    anuncio puede no coincidir con el total de campaña cuando una misma
    conversión se atribuye a más de un anuncio dentro de la campaña
    (solapamiento de atribución) — así el número que más se mira queda
    siempre fiel a lo que muestra Meta."""
    if not AD_ACCOUNT_ID:
        raise MetaAPIError("Falta META_AD_ACCOUNT_ID.")
    if range_key not in DATE_PRESETS:
        raise MetaAPIError(f"Rango de fecha desconocido: {range_key}")

    cache_key = f"campaign:{range_key}"
...
def get_market_summary(market: str, range_key: str) -> dict:
    """Resumen exacto (gasto, SQL, costo por resultado) a nivel campaña
    para el mercado pedido — este es el número que se muestra arriba del
    todo en el dashboard, y coincide con la pestaña Campañas de Meta."""
    if market not in MARKET_PREFIXES:
        raise MetaAPIError(f"Mercado desconocido: {market}")
    prefix = MARKET_PREFIXES[market]
    matched = [
        r for r in _fetch_campaign_level(range_key)
        if r["campaign_name"].upper().startswith(prefix)
    ]
    total_spend = sum(r["spend"] for r in matched)
    total_sql = sum(r["sql"] for r in matched)
    return {
        "spend": total_spend,
        "sql": total_sql,
        "cost_per_result": round(total_spend / total_sql, 2) if total_sql > 0 else None,
    }
