...
def _pick_result(actions: list[dict] | None, sql_action_type: str | None) -> tuple[str, float]:
    ...
    Confirmado en Meta Events Manager: HubSpot le manda a este píxel
    todo el embudo de etapas (unqualified_lead, marketingqualifiedlead,
    salesqualifiedlead, opportunity, customer). Ninguna de las otras
    está formalizada como conversión personalizada, así que Meta las
    agrupa TODAS bajo "offsite_conversion.fb_pixel_custom" — usar ese
    balde como respaldo no es una aproximación, es sumar SQL junto con
    oportunidades y clientes, un número estructuralmente equivocado.

    Por eso acá SOLO se confía en la conversión personalizada formal
    "SQL" (ver _get_sql_action_type). Cuando no aparece para un anuncio
    puntual, es 0 — puede ser un 0 real, o puede ser que Meta todavía no
    terminó de indexar esa conversión para ese anuncio/día (se vio
    inconsistente en pruebas). Mejor un 0 ocasional que un número
    mezclado con otras etapas del funnel."""
...
def _fetch_insights(account_id: str, date_preset: str) -> dict:
    ...
    data = _get(
        f"act_{account_id}/insights",
        {
            ...
            "fields": fields,
            # Sin esto, la API puede calcular las conversiones con una
            # ventana de atribución distinta a la configurada en la
            # cuenta, y los números no coinciden con los de Ads Manager.
...
def _fetch_campaign_level(range_key: str) -> list[dict]:
    ...
    rows = []
    for row in data.get("data", []):
        _, sql_count = _pick_result(row.get("actions"), sql_action_type)
        rows.append(
            {
                "campaign_name": row.get("campaign_name", ""),
                "spend": float(row.get("spend", 0)),
